...
- 在弹幕中输入短句（默认“查询盲盒”）可查询心动盲盒投入与开出礼物价值的盈亏，并自动回复弹幕（盲盒基础礼物不入库，开出的礼物按 ¥15/个计算）。
...
- `BLIND_BOX_TRIGGERS`：触发短句，逗号或换行分隔，默认 `查询盲盒`。
...
- 盲盒的基础礼物不会入库也不会计入流水，盲盒开出的礼物会正常入库并计入流水，且在盈亏中按单价 ¥15 统计。
